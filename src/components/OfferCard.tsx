...
export function OfferCard({ offer }: { offer: Offer }) {
  ...
  const slug = `/offers/${slugify(offer.title)}-${offer.id}`;
  ...
  return (
    ...
    >
      <Link href={slug} className="block w-full h-44 relative bg-orange-50">
      ...
      <div className="p-4 flex flex-col flex-1 gap-2">
        <Link href={slug}>
          <h3 className="text-lg font-bold text-orange-600 hover:underline" itemProp="name">
            {offer.title}
          </h3>
        </Link>
      ...
      <div className="p-4 pt-2 flex gap-2 mt-auto">
        {whatsappLink && (
          <a href={whatsappLink} target="_blank" rel="noopener" className="flex items-center gap-1 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-full text-sm font-bold shadow">
            <MessageCircle size={16} />
            واتساب
          </a>
        )}
        {phoneLink && (
          <a href={phoneLink} className="flex items-center gap-1 px-3 py-2 bg-primary hover:bg-primary/90 text-white rounded-full text-sm font-bold shadow">
            <Phone size={16} />
            اتصال
          </a>
        )}
